...
def main():
    parser = argparse.ArgumentParser(description="Track persons in a video with YOLO+BoT-SORT")
    parser.add_argument("--input",    default=VIDEO_PATH,       help="Input video path")
    parser.add_argument("--out-csv",  default=OUTPUT_CSV_PATH,  help="Output tracking CSV")
    parser.add_argument("--out-video",default=OUTPUT_VIDEO_PATH,help="Output annotated video")
    parser.add_argument("--out-traj", default=OUTPUT_TRAJECTORY_VIDEO_PATH,
                        help="Output trajectory-only video")
    parser.add_argument("--out-summary", default=OUTPUT_SUMMARY_CSV_PATH,
                        help="Output track summary CSV")
    parser.add_argument("--no-display", action="store_true",
                        help="Disable live preview window (for headless/batch use)")
    ...
    traj_writer = cv2.VideoWriter(out_traj, fourcc, fps, (width, height))
    ...
    with open(out_summary, "w", newline="") as sf:
    ...
    print(f"Saved tracked video to:    {out_video}")
    print(f"Saved trajectory video to: {out_traj}")
    print(f"Saved tracks CSV to:       {out_csv}")
    print(f"Saved track summary CSV to:{out_summary}")
